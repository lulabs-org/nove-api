...
**A:** 通过 `POST /api/auth/otp/send` 发送邮箱或手机验证码。请求字段和有效期以 Swagger 与当前环境配置为准。
...
**A:** 目前系统深度集成了以下平台：
- **腾讯会议** — 支持开放 API、事件回调、录制/转写数据同步与 AI 总结
- **飞书会议** — 支持事件回调与多维表格数据同步

### Q: 会议录制和转写功能如何使用？

**A:** 会议录制和转写来源于集成平台。系统接收 Webhook 后同步录制、转写和智能总结元数据；具体文件是否可下载取决于平台响应与当前存储配置。可通过会议、录制和转写接口查询结果。
...
**A:**
1. 查看本 FAQ 页面
2. 启动服务后打开 `http://localhost:3000/api` 了解 API 细节

...
本文档详细描述了 Nove API 系统中的各种数据流处理流程，包括事件处理、文件处理、数据同步和业务流程。

## 腾讯会议事件处理流程

```mermaid
flowchart TD
    A["1. 腾讯会议平台发送 Webhook 事件"] --> B["2. TencentWebhookController 接收并验证"]
    B --> B1{"请求签名验证"}
    B1 -->|失败| B1E["返回 401"]
    B1 -->|通过| B2{"请求体解密"}
    B2 -->|失败| B2E["返回 400"]
    B2 -->|通过| B3{"事件格式验证"}
    B3 -->|失败| B3E["返回 422"]
    B3 -->|通过| C["3. TencentEventHandlerService 事件分发"]
    C --> C1{"事件类型识别"}
    C1 -->|未知| C1E["记录日志并跳过"]
    C1 -->|已知| D["4. EventHandlerFactory 创建处理器"]
    D --> E["5. 具体事件处理器执行业务逻辑"]
    E --> E1["数据解析和验证"]
    E --> E2["业务规则检查"]
    E --> E3["数据转换和映射"]
    E1 & E2 & E3 --> F["6. 数据持久化到 PostgreSQL"]
    F --> F1{"事务处理"}
    F1 -->|失败| F1E["回滚并记录错误"]
    F1 -->|成功| G{"7. 同步到飞书 Bitable?"}
    G -->|是| G1["创建异步同步任务入队"]
    G -->|否| H["8. 返回处理结果"]
    G1 --> H
```

...
export class PasswordService {
  ...
  async resetPassword(
    ...
    // 先执行 Redis 拉黑再更新密码：若 Redis 不可用则直接失败，密码尚未变更，
    // 用户可用原密码重试，避免"密码已改但旧会话未吊销"的不一致状态。
    // 注意：JTI 拉黑只能覆盖"最新一次签发"的 access token；轮换前的旧 access token
    // 与存量 jti=null 会话由 tokenVersion 失效边界 + 轮换版本护栏统一兜底。
    ...
    // Redis 黑名单写入成功后，在同一数据库事务内完成：
    // ① updatePassword：更新密码哈希并原子递增 tokenVersion（用户级失效边界）
    // ② revokeAllTokensByUserId：撤销全部未撤销的 refresh token
    //
    // 事务与 refresh 轮换的"原子消费"（consumeToken 条件更新）在行级互斥：
    // - 并发轮换先消费旧行 → 本事务的 updateMany 在行锁释放后重新评估，
    //   会把轮换新建的记录一并撤销；
    // - 本事务先撤销旧行 → 并发轮换的消费条件不再匹配（count=0）→ 轮换被拒。
    // 即使仍有记录漏网（极端交错），refresh 轮换的版本护栏
    // （record.tokenVersion !== user.tokenVersion → 拒绝）会阻断其换发，
    // 旧 access token 也因 ver 不一致被 JwtStrategy 拒绝——三重防线闭环。
    await this.prisma.$transaction(async (tx) => {
      await this.userCommandRepo.updatePassword(user.id, hashedPassword, tx);

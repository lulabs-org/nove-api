  ...
  async updateRule(id: string, dto: UpdateProfitShareRuleDto) {
    try {
      return await this.ruleRepository.updateWithDetails(id, {
        ...
        productId: dto.productId,
        channelId: dto.channelId,
        validStartTime: dto.validStartTime ? new Date(dto.validStartTime) : undefined,
        ...
        ...(dto.modules && {
          modules: {
            deleteMany: {
              id: { notIn: dto.modules.map(m => m.id).filter(Boolean) as string[] }
            },
            update: dto.modules.filter(m => m.id).map(m => ({
              where: { id: m.id },
              data: {
                name: m.name,
                ...
                allocations: {
                  deleteMany: {
                    id: { notIn: (m.allocations || []).map(a => a.id).filter(Boolean) as string[] }
                  },
                  update: (m.allocations || []).filter(a => a.id).map(a => ({
                    where: { id: a.id },
                    data: {
                      memberId: a.memberId,
                      roleId: a.roleId,
                  ...
                  create: (m.allocations || []).filter(a => !a.id).map(a => ({
                    memberId: a.memberId,
                    roleId: a.roleId,
                    allocationRatio: new Decimal(a.allocationRatio),
                  }))
            ...
            create: dto.modules.filter(m => !m.id).map(module => ({
              name: module.name,
              shareRatio: new Decimal(module.shareRatio),
              isRefundable: module.isRefundable ?? true,
              amortizationType: module.amortizationType ?? 'NONE',
              allocations: {
                create: module.allocations ? module.allocations.map(allocation => ({
                  memberId: allocation.memberId,
                  roleId: allocation.roleId,
                  allocationRatio: new Decimal(allocation.allocationRatio),
                })) : [],

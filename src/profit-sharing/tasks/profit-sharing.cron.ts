...
export class ProfitSharingCron {
  ...
  constructor(private readonly recordRepository: ProfitSharingRecordRepository) {}
  ...
  async handleSettlement() {
    ...
    try {
      const result = await this.recordRepository.updatePendingRecordsToSettled();

      this.logger.log(`Automatic settlement completed. Updated ${result.count} records to SETTLED.`);
    } catch (error) {
      this.logger.error('Failed to execute automatic settlement cron job', error);
    }
  }
}

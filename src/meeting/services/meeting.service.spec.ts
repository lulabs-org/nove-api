...
describe('MeetingService', () => {
  ...
  it('rejects detail, mutation and participants for an inaccessible meeting', async () => {
    ...
    repository.exists.mockResolvedValue(false);
    await expect(
      service.findById('other-org-meeting', 'org-1'),
    ).rejects.toBeInstanceOf(MeetingRecordNotFoundException);
    await expect(
      service.update('other-org-meeting', { title: 'Changed' }, 'org-1'),
    ).rejects.toBeInstanceOf(MeetingRecordNotFoundException);
    await expect(
      service.delete('other-org-meeting', 'org-1'),
    ).rejects.toBeInstanceOf(MeetingRecordNotFoundException);
    await expect(
      service.findParticipants('other-org-meeting', {}, 'org-1'),
    ).rejects.toBeInstanceOf(MeetingRecordNotFoundException);

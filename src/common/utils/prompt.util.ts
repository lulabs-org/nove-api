const PROMPT_TEMPLATES = {
  PARTICIPANT_SUMMARY: {
    system: '你是专业的会议总结助手，擅长根据会议背景和发言记录，为特定的参会者提炼结构化、个性化的总结。请使用 Markdown 格式输出。',
...
- 如果某项内容（如个人行动项）在片段中未体现，请直接标明“无相关记录”，不要凭空捏造。`,
  ...
  PERIOD_SUMMARY: {
    system: '你是一位专业的 AI 会议总结助手，擅长将个人的多次零散总结提炼为一份高质量的阶段性（如周报、月报）聚合总结。请务必使用 Markdown 格式输出。',
